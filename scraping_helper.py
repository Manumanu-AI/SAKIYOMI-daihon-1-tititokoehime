...
# シミラリティ検索を実行する関数
def perform_similarity_search(index, query, namespace, top_k=2):
...
def generate_response_with_llm_for_multiple_namespaces(index, user_input, namespaces):
    ...
    ・CTAに「そんなここは、(場所の名前)」という感じで場所を紹介して、サクッと締めてください。CTA内でしか場所の名前は言わないこと。
    ・見ている人がその場所に行きたくなるような内容にして下さい。
    ・一般的な情報を載せるだけではつまらないので、主観的に言い切ったり、紹介する要素を限定して尖らせて下さい。その方が人間らしく、面白いリールになります。

    ----------
    生成する台本のフォーマットは【アウトプット例】と同じにして生成してください。
    これは過去に私が作ったものなので、そちらを再現するように口調、文章量、表現を真似て下さい。
    ----------
    【アウトプット例】
    {example_plot}
    ...
    注意点として、あくまでも「情報ソース」としてのみ使用すること、台本の形や雰囲気は【アウトプット例】と【その他の過去に作った台本】を遵守すること。遵守しないとペナルティがあります。
